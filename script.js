...
// Tooltip per il campo Peso Attuale (PRIMO tooltip - id: tooltipIcon / tooltipText)
const tooltipIcon = document.getElementById('tooltipIcon');
const tooltipText = document.getElementById('tooltipText');

if (tooltipIcon && tooltipText) {
  tooltipIcon.addEventListener('mouseenter', () => tooltipText.classList.add('show'));
  tooltipIcon.addEventListener('mouseleave', () => tooltipText.classList.remove('show'));
}
...
function toggleVisibility(containerId, button, showTextKey, hideTextKey) {
  const container = document.getElementById(containerId);
  const isHidden = container.classList.toggle('hidden');
  const lang = document.documentElement.getAttribute('data-ui-lang') || 'it';

  const showText = button.getAttribute(lang === 'it' ? `data-text-it` : `data-text-en`);
  const hideText = button.getAttribute(lang === 'it' ? `data-hide-it` : `data-hide-en`);
